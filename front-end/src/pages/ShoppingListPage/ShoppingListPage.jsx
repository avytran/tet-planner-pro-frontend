...
export default function ShoppingListPage() {
  ...
  const rawItems = (currentData?.getShoppingItemsOfUser?.items || []).map(
    (item) => ({
      ...item,
    }),
  );
  const maxPrice =
    rawItems.length > 0
      ? Math.max(...rawItems.map((i) => i.price || 0))
      : 5000000;
  ...
  const budgetCategoryObjects = budgets
    .map((b) => ({ id: b?.id, name: b?.name }))
    .filter((b) => b.id && b.name);
  const totalShoppingCostRaw = rawItems.reduce(
    (sum, item) => sum + item.price * item.quantity,
    0,
  );
  const shoppingCompletedRaw =
    rawItems.length > 0
      ? Math.round(
          (rawItems.filter((i) => i.status === "Completed").length /
            rawItems.length) *
            100,
        )
      : 0;
  const totalCategoryCostRaw = budgets.map((budget) => {
    const spent = rawItems
      .filter((i) => i.budget?.id === budget.id)
            ...
            <div className="flex-1 flex flex-col gap-4 min-w-0">
              ...
              <div className="overflow-x-auto">
                <div className="min-w-full">
                  <ShoppingTable items={shoppingItems} refetch={refetch} />
                </div>
                {/* Pagination */}
            ...
            <div className="w-full md:w-[340px] min-w-[180px] flex flex-col gap-4 flex-shrink md:ml-0 ml-0 md:mt-0 mt-6 items-stretch">
              ...
              {totalCategoryCostRaw.map((cat, idx) => (
                <CategoryCostWidget
                  ...
                  percent={
                    cat.total > 0
                      ? Math.min(100, Math.round((cat.spent / cat.total) * 100))
                      : 0
                  }
                  spent={cat.spent}
                  total={cat.total}
